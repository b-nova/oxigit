...
async fn test_session_detail_shows_operations() {
    ...
    client
        .create_repo("opsrepo", "Session ops test", false)
        .await;
    ...
    assert!(
        body.contains("Session Operations"),
        "Expected operations panel, got: {}",
...
async fn test_session_detail_content() {
    ...
    assert!(
        body.contains("Create hello function"),
        "Expected first prompt"
    );
    assert!(
        body.contains("Create world function"),
        "Expected second prompt"
    );
}
...
async fn test_session_ops_hidden_for_non_owner() {
    ...
    client.login("alice", "password123").await;
    client
        .create_repo("privops", "Private ops test", false)
        .await;
