...
async fn collaborator_can_push() {
    ...
    assert!(output.status.success(), "bob should be able to clone public repo");
    init_repo_config(&bob_dir);
    create_commit(&bob_dir, "bob.txt", "bob was here", "Bob commit");
    let output = git_push(&bob_dir);
    assert!(!output.status.success(), "bob should NOT be able to push without collaborator access");
    ...
    // Bob can NOW push
    let output = git_push(&bob_dir);
    assert!(output.status.success(), "bob should be able to push as collaborator: {}", String::from_utf8_lossy(&output.stderr));
}
...
async fn remove_collaborator() {
    ...
    let resp = alice.remove_collaborator_by_name("alice", "myrepo", "bob").await;
    assert!(resp.status().is_success() || resp.status().is_redirection());

    let resp = alice.list_collaborators("alice", "myrepo").await;
    let body = resp.text().await.unwrap();
    assert!(!body.contains("bob") || body.contains("[]"), "bob should be removed");
}
