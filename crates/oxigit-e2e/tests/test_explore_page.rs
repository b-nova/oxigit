...
async fn explore_lists_public_repos() {
    ...
    client.create_repo("public-one", "First public repo", false).await;
    client.create_repo("public-two", "Second public repo", false).await;
    ...
    assert!(body.contains("public-one"), "should list first public repo: {body}");
    assert!(body.contains("public-two"), "should list second public repo: {body}");
}
...
async fn explore_hides_private_repos() {
    ...
    assert!(!body.contains("hidden"), "should NOT show private repo: {body}");
}
...
async fn explore_search_by_name() {
    ...
    client.register("alice", "alice@test.com", "password123").await;
    client.create_repo("rust-project", "A Rust project", false).await;
    client.create_repo("go-project", "A Go project", false).await;
    ...
    assert!(!body.contains("go-project"), "should NOT find go-project: {body}");
}
...
async fn explore_search_by_owner() {
    ...
    assert!(body.contains("Alice repo"), "should contain alice's description: {body}");
}
