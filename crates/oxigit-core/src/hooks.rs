...
pub async fn process_post_receive(
    ...
    for (refname, new_sha) in after_refs {
        ...
        let guardrail_rules = db::get_guardrail_rules(pool, repo_id)
            .await
            .unwrap_or_default();
        let guardrail_config = db::get_guardrail_config(pool, repo_id).await.ok().flatten();
        let warn_rules: Vec<_> = guardrail_rules
            .iter()
        ...
        if !warn_rules.is_empty()
            || guardrail_config
                .as_ref()
                .map(|c| c.max_files_per_push.is_some())
                .unwrap_or(false)
        {
            for sha in &shas {
                let diff = git::show_commit_diff(repo_path, sha)
                    ...
                    .map(|(_, d)| d)
                    .unwrap_or_default();
                let file_count = git::list_changed_files(repo_path, sha)
                    .unwrap_or_default()
                    .len();

                let violations =
                    guardrail::evaluate_diff(&warn_rules, &guardrail_config, &diff, file_count);
                let warn_violations: Vec<_> =
                    violations.iter().filter(|v| v.action == "warn").collect();
