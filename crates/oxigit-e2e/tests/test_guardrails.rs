...
async fn test_guardrail_settings_visible_to_owner() {
    ...
    client
        .create_repo("guardrepo", "Guardrail test", false)
        .await;
    ...
    assert!(
        body.contains("AI Guardrails"),
        "Expected guardrails section in settings, got: {}",
...
async fn test_guardrail_off_allows_everything() {
    ...
    assert!(
        push.status.success(),
        "All guardrails off should allow everything"
    );
}
...
async fn test_violations_on_ai_hub() {
    ...
    client
        .register("frank", "frank@test.com", "password123")
        .await;
    client.login("frank", "password123").await;
    client
        .create_repo("hubviolation", "Hub violation test", false)
        .await;
