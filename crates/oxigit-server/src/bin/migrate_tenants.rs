...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !legacy_path.exists() {
        eprintln!("No oxigit.db found at {}", legacy_path.display());
        std::process::exit(1);
    }

    let control_path = data_dir.join("control.db");
    if control_path.exists() {
        eprintln!("control.db already exists — migration may have already run.");
        eprintln!("Remove it manually if you want to re-run.");
        std::process::exit(1);
    }
    ...
    println!("Opened legacy database at {}", legacy_path.display());
    ...
    oxigit_core::db::run_control_migrations(&control).await?;

    println!("Created control.db with schema");
    ...
    println!("Control-plane data copied.");
    ...
    for (user_id, username, ..) in &users {
        let user_id = *user_id;
        println!("  Provisioning tenant for user '{}'...", username);
        ...
        oxigit_core::db::run_tenant_migrations(&tenant).await?;
                ...
                for entry in entries.flatten() {
                    ...
                    if src.is_dir() {
                        ...
                        if let Err(e) = std::fs::rename(&src, &dest) {
                            ...
                            eprintln!("    rename failed ({}), copying instead...", e);
        ...
        tenant.close().await;
        println!("    {} repos migrated", repos.len());
    }
    ...
    println!("\nMigration complete!");
    println!("  Legacy DB backed up to: {}", backup_path.display());
    println!("  Control DB: {}", control_path.display());
    println!("  Tenants: {}/tenants/*/tenant.db", data_dir.display());
...
async fn copy_table_raw(
    ...
    println!("  Copied {} rows from '{}'", rows.len(), table);
    Ok(())
}
