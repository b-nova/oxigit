...
async fn test_create_public_repo() {
    ...
    let resp = client.create_repo("myproject", "A test project", false).await;
    assert!(
...
async fn test_create_private_repo() {
    ...
    let _ = client.register("alice", "alice@example.com", "password123").await;
    let resp = client.create_repo("secret-project", "Private repo", true).await;
    assert!(
        resp.status().is_redirection() || resp.status().is_success(),
...
async fn test_create_repo_unauthenticated() {
    ...
    assert!(
        body.contains("error") || body.contains("Error") || body.contains("authenticated") || body.contains("login"),
...
async fn test_create_duplicate_repo() {
    ...
    let body = resp.text().await.unwrap();
    assert!(
        body.contains("error") || body.contains("Error") || body.contains("exists") || body.contains("already"),
